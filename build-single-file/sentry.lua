...
\
\
   local envelope_body = envelope.build_error_envelope(event)\
   local success, err = (self.transport):send_envelope(envelope_body)\
\
   if self.options.debug then\
      if success then\
         print(\"[Sentry] Event sent via envelope: \" .. event.event_id)\
      else\
         print(\"[Sentry] Failed to send event envelope: \" .. tostring(err))\
      end\
   end\
\
...
\
   if self.options.debug then\
      if success then\
         print(\"[Sentry] Exception sent via envelope: \" .. event.event_id)\
      else\
         print(\"[Sentry] Failed to send exception envelope: \" .. tostring(err))\
      end\
   end\
\
   return success and event.event_id or \"\"\
...
\
         if client.transport then\
            local envelope_body = envelope.build_error_envelope(event)\
            local success, err = client.transport:send_envelope(envelope_body)\
            if client.options.debug then\
               if success then\
                  print(\"[Sentry] Fatal error sent via envelope: \" .. event.event_id)\
               else\
                  print(\"[Sentry] Failed to send fatal error envelope: \" .. tostring(err))\
               end\
            end\
\
\
...
\
\
function NginxTransport:send_envelope(envelope_body)\
   local request = {\
...
\
\
function RobloxTransport:send_envelope(envelope_body)\
\
   if not _G.game then\
...
\
   local success, response = pcall(function()\
      return HttpService:PostAsync(self.envelope_endpoint, envelope_body,\
      (_G).Enum.HttpContentType.TextPlain,\
      false,\
      self.envelope_headers)\
\
   end)\
\
   if success then\
      return true, \"Envelope sent via Roblox HttpService\"\
   else\
...
\
\
local function build_error_envelope(event)\
\
   local sent_at = os.date(\"!%Y-%m-%dT%H:%M:%SZ\")\
\
\
...
\
   ...
   local payload_length = string.len(event_json)\
\
\
   local item_header = {\
      type = \"event\",\
...
return {\
   ...
   build_error_envelope = build_error_envelope,\
}\
