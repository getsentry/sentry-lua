...
local function send_confirmation_email(user_email, order_id, amount) sentry.capture_message("Purchase confirmation sent to " .. user_email .. " for order #" .. order_id, "info") end
...
local function validate_cart(items_count, total_price)
  if items_count > 0 then process_payment("credit_card", total_price, "USD") end
end
...
sentry.with_scope(function(scope)
  ...
  local function safe_array_access() array_access_error("engineering", 2) end
...
sentry.with_scope(function(scope)
  ...
  local function safe_string_concat() string_concat_error("Hello", "john_doe", 42) end
...
sentry.with_scope(function(scope)
  ...
  local function safe_nil_access() nil_method_call("database", "production") end
...
sentry.with_scope(function(scope)
  ...
  if result:find("processed 0/") then sentry.capture_message("Table iteration produced empty result - likely using wrong iterator", "warning") end
end)
...
local function fetch_user_data(user_id, include_preferences) database_query("SELECT", "users", 5000) end

local function authenticate_user(username, password_hash) fetch_user_data("user_12345", true) end

local function handle_request(request_id, client_ip) authenticate_user("john.doe", "sha256_abc123") end
