...
function love.update(dt)
  ...
  button.hover = (mouse_x >= button.x and mouse_x <= button.x + button.width and mouse_y >= button.y and mouse_y <= button.y + button.height)
...
function love.draw()
  ...
  if button.pressed then button_color = { 1.0, 0.3, 0.3, 1 } end
  ...
  love.graphics.print(button.text, button.x + (button.width - text_width) / 2, button.y + (button.height - text_height) / 2)

  -- Draw fatal button
  local fatal_button = game.fatal_button
  local fatal_button_color = fatal_button.hover and { 0.8, 0.2, 0.8, 1 } or { 0.6, 0.1, 0.6, 1 }
  if fatal_button.pressed then fatal_button_color = { 1.0, 0.3, 1.0, 1 } end
  ...
  love.graphics.print(fatal_button.text, fatal_button.x + (fatal_button.width - fatal_text_width) / 2, fatal_button.y + (fatal_button.height - text_height) / 2)
  ...
  if game.last_error_time > 0 then love.graphics.print(string.format("Last error: %.1fs ago", love.timer.getTime() - game.last_error_time), 400, love.graphics.getHeight() - 40) end
end
...
function love.mousepressed(x, y, button_num, istouch, presses)
  if button_num == 1 then -- Left mouse button
    ...
    if x >= button.x and x <= button.x + button.width and y >= button.y and y <= button.y + button.height then
      ...
      xpcall(function() game.demo_functions.level1("button_click", "button_click") end, error_handler)

      -- Check if click is within fatal button bounds
    elseif x >= game.fatal_button.x and x <= game.fatal_button.x + game.fatal_button.width and y >= game.fatal_button.y and y <= game.fatal_button.y + game.fatal_button.height then
