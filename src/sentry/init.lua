...
if type(debug) == "table" and type(debug.getinfo) == "function" then
  ...
  if info and type(info.source) == "string" and info.source:sub(1, 1) == "@" then
    ...
    if local_dir then package.path = local_dir .. "?.lua;" .. local_dir .. "?/init.lua;" .. package.path end
  end
end
...
local function with_scope(callback)
  ...
  if not success then logger.error("callback failed to run through with_scope: " .. result) end
end
