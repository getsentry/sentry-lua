...
function Transport:_process_queue()
    if self.processing or #self.queue == 0 or self.active_requests >= self.max_concurrent then
        return
    end
            ...
            function(ok, status, body)
                ...
                if not ok then
                    ...
                    if body and body ~= "" then
                        msg = msg .. ", response: " .. tostring(body)
                    end
                    logger.error(msg)
    ...
    if #self.queue > 0 and self.active_requests < self.max_concurrent then
        self:_process_queue()
    end
end
