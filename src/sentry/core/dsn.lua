...
local function parse_dsn(dsn_string)
  if not dsn_string or dsn_string == "" then return nil, "DSN is required" end
  ...
  local port = 443
  if protocol == "http" then port = 80 end

  local host_part, port_part = host:match("^([^:]+):?(%d*)$")
  if host_part then
    host = host_part
    if port_part and port_part ~= "" then port = tonumber(port_part) or port end
  end
...
local function build_envelope_url(dsn) return string.format("%s://%s/api/%s/envelope/", dsn.protocol, dsn.host, dsn.project_id) end
...
local function build_auth_header(dsn)
  ...
  if dsn.secret_key and dsn.secret_key ~= "" then table.insert(auth_parts, "sentry_secret=" .. dsn.secret_key) end
